...
public class PageMaker {
	...
	// 조회한 전체 항목을 설정할 때 각 멤버 변수를 초기화 수행
	...
	public void memberInit() {
		...
		// 시작 페이지 계산 : 끝페이지 번호 - 표시할 페이지 수 + 1
		startPage = endPage - displayPageNum + 1;
		
		// 실제 마지막 페이지 계산 : 올림(조회한 전체 항목의 수 / 페이지당 출력 항목의 수)
		realEndPage = (int)Math.ceil(totalCount / (double)cri.getNumPerPage());
		
		// 실제 마지막 페이지(realEndPage)와 계산한 마지막 페이지(endPage)가 값이 틀리면 마지막 페이지를 realEndPage로 수정
		if (endPage > realEndPage) {
			endPage = realEndPage;
		}
		
		prev = (startPage == 1 ? false : true);
		// 조회한 전체 항목 수가 마지막 페이지 * 페이지 당 표시 항목 수보다 크면 다음 페이지가 존재
		next = totalCount > (endPage * cri.getNumPerPage()) ? true : false;
	}
	
	// 페이지 번호를 입력 받아 admin_product_list의 쿼리 스트링을 만들어 주는 메소드
	// 입력 매개변수 :
	//		page : 표시할 페이지 번호
	// 출력 예 :
